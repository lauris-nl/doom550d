...
void G_DoSaveGame (void)
{
    ...
    fprintf(stderr, "save: header\n");


    P_WriteSaveGameHeader(savedescription);


    fprintf(stderr, "save: players\n");
    ...
    fprintf(stderr, "save: specials\n");
    P_ArchiveSpecials();

    fprintf(stderr, "save: eof\n");

    P_WriteSaveGameEOF();


    fprintf(stderr, "save: flush-close\n");
    ...
    // Finish up. The compatibility layer buffers the many one-byte writes.
    // Flush the final block and propagate delayed card-write errors.

    if (fflush(save_stream) != 0)
        savegame_error = true;
    ...
    fprintf(stderr, "save: rename %s -> %s\n", temp_savegame_file, savegame_file);
